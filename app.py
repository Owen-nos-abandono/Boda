...
# ──────────────────────────────────────────────────────────
#  BASE DE DATOS: Postgres en producción (Vercel), SQLite en local
#  Se activa Postgres automáticamente si existe la variable DATABASE_URL
# ──────────────────────────────────────────────────────────
DATABASE_URL = (
...
class DBWrapper:
    """Pequeña capa que permite usar la misma sintaxis (con '?') tanto
    para SQLite (local) como para Postgres (Vercel)."""
    ...
    def execute(self, query, params=()):
        q = query.replace("?", "%s") if self.is_pg else query
...
WEDDING = {
    "bride": "Janine",
    "groom": "Iván",
    "bride_full": "Janine Rivera Díaz",
    "groom_full": "Eddi Iván Morales Zequera",
    "bride_parents": {
        "father": "Luis Gerardo Rivera González",
        "mother": "Claudia Díaz Lule",
    },
    "groom_parents": {
        "father": "Heriberto Morales Gallardo",
        "mother": "Olivia Zequera López",
    },
    "registry_url": "https://www.amazon.com.mx/wedding/guest-view/2WH32SR40W621",
...
def get_stats(db):
    total     = db.execute("SELECT COUNT(*) AS c FROM rsvp").fetchone()["c"]
    row       = db.execute(
        "SELECT COUNT(*) AS c, COALESCE(SUM(guests),0) AS g FROM rsvp WHERE attending=1"
    ).fetchone()
    attending = row["c"]
    total_pax = row["g"]
    not_att   = db.execute("SELECT COUNT(*) AS c FROM rsvp WHERE attending=0").fetchone()["c"]
...
def admin_delete(rsvp_id):
    db = get_db()
    db.execute("DELETE FROM rsvp WHERE id=?", (rsvp_id,))
    db.commit()
